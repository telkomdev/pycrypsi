import os
import unittest
from pycrypsi import rsa

BASE_DIR = os.path.dirname(os.path.abspath(__file__))

class TestRSA(unittest.TestCase):

    def test_generate_key_pairs_1kb(self):
        private_key, public_key, private_key_b64, public_key_b64 = rsa.generate_key_pairs(rsa.KEY_SIZE_1KB)
        self.assertIsNotNone(private_key)
        self.assertIsNotNone(public_key)
        self.assertIsNotNone(private_key_b64)
        self.assertIsNotNone(public_key_b64)

    def test_generate_key_pairs_2kb(self):
        private_key, public_key, private_key_b64, public_key_b64 = rsa.generate_key_pairs(rsa.KEY_SIZE_2KB)
        self.assertIsNotNone(private_key)
        self.assertIsNotNone(public_key)
        self.assertIsNotNone(private_key_b64)
        self.assertIsNotNone(public_key_b64)

    def test_generate_key_pairs_4kb(self):
        private_key, public_key, private_key_b64, public_key_b64 = rsa.generate_key_pairs(rsa.KEY_SIZE_4KB)
        self.assertIsNotNone(private_key)
        self.assertIsNotNone(public_key)
        self.assertIsNotNone(private_key_b64)
        self.assertIsNotNone(public_key_b64)

    def test_load_private_key_from_base64(self):
        base64_rsa_private_key = ''
        private_key = rsa.load_key_from_base64(bytes(base64_rsa_private_key, 'utf-8'))
        self.assertIsNotNone(private_key)
            
    def test_load_public_key_from_base64(self):
        base64_rsa_public_key = ''
        public_key = rsa.load_key_from_base64(bytes(base64_rsa_public_key, 'utf-8'))
        self.assertIsNotNone(public_key)

    def test_load_private_key_from_file(self):
        private_key_path = os.path.join(BASE_DIR, 'testdata/private.key')
        with open(private_key_path) as private_key_file:
            private_key = rsa.load_key(private_key_file.read())
            self.assertIsNotNone(private_key)
    
    def test_load_public_key_from_file(self):
        public_key_path = os.path.join(BASE_DIR, 'testdata/public.key')
        with open(public_key_path) as public_key_file:
            public_key = rsa.load_key(public_key_file.read())
            self.assertIsNotNone(public_key)
    
    def test_load_private_key_from_file_should_throw_error_with_invalid_key(self):
        private_key_path = os.path.join(BASE_DIR, 'testdata/private_key_invalid.key')
        with open(private_key_path) as private_key_file:
            with self.assertRaises(Exception):
                private_key = rsa.load_key(private_key_file.read())
                self.assertIsNotNone(private_key)
    
    def test_load_public_key_from_file_should_throw_error_with_invalid_key(self):
        public_key_path = os.path.join(BASE_DIR, 'testdata/public_key_invalid.key')
        with open(public_key_path) as public_key_file:
            with self.assertRaises(Exception):
                public_key = rsa.load_key(public_key_file.read())
                self.assertIsNotNone(public_key)